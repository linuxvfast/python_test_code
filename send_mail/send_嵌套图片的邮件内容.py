...
def addimg(src,imgid):
    '''add image
    src:image path
    imgid:image match args'''
    fp = open(src, 'rb')
    msgImage = MIMEImage(fp.read())
    fp.close()
    msgImage.add_header('Content-ID', imgid)#img标签的src属性时通过Content-ID来引用的，所有要用Content-ID
    return msgImage

msg = MIMEMultipart('related')# related内嵌图片
msgtext = MIMEText("""
